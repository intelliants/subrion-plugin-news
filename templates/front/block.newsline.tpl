					...
					<div class="media ia-item ia-item--border-bottom">
						<div class="media-body">
							{if $entry.image && $core.config.newsline_display_img}
								<a href="{$smarty.const.IA_URL}news/{$entry.id}-{$entry.alias}" class="pull-right">{printImage imgfile=$entry.image title=$entry.title width='100'}</a>
							{/if}
							<h4 class="media-heading">
								<a href="{$smarty.const.IA_URL}news/{$entry.id}-{$entry.alias}">{$entry.title}</a>
							</h4>
							<p class="text-fade-50">{$entry.date|date_format:$core.config.date_format} {lang key='by'} {$entry.fullname}</p>
